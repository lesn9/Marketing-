...
class AIEngine:
    ...
    async def chat(self, system: str, user: str, model: str = "llama-3.3-70b-versatile") -> str:
        ...
        if self.groq_key:
            providers.append(
                ("groq", self.groq_key, "https://api.groq.com/openai/v1/chat/completions", "llama-3.3-70b-versatile")
            )
        if self.openrouter_key:
            providers.append(
                ("openrouter", self.openrouter_key, "https://openrouter.ai/api/v1/chat/completions", "meta-llama/llama-3.3-70b-instruct")
            )
    ...
    async def _call(self, url: str, key: str, system: str, user: str, model: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if resp.status_code != 200:
                raise AIProviderError("AI_PROVIDER_ERROR", f"HTTP {resp.status_code}: {resp.text[:200]}")
